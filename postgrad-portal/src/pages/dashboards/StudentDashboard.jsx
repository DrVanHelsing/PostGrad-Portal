// ============================================
// Student Dashboard – condensed milestones, calendar widget, no profile card
// ============================================
...
export default function StudentDashboard() {
  const { user } = useAuth();
  const { getRequestsByStudent, getStudentProfile, mockMilestones, addMilestone, updateMilestone, deleteMilestone } = useData();
  ...
  const handleEditMilestone = async () => {
    ...
    try {
      await updateMilestone(editingMilestone.id, { title: editMsForm.title, type: editMsForm.type, date: new Date(editMsForm.date), description: editMsForm.description });
  ...
  const handleDeleteMilestone = async (msId) => {
    ...
    try { await deleteMilestone(msId); showToast('Milestone deleted'); }
    catch (err) { showToast(err.message || 'Failed to delete', 'error'); }
  };

  return (
    <div className="page-wrapper main-dashboard-compact">
      ...
      <div style={{ marginBottom: 'var(--space-2xl)' }}>
        <div className="quick-actions-grid compact-quick-actions-grid">
          ...
          <div className="quick-action-card" onClick={() => navigate('/progress-tracker')}>
            <div className="quick-action-icon" style={{ background: 'var(--status-purple-bg)', color: 'var(--status-purple)' }}><HiOutlineClock /></div>
            <span className="quick-action-label">Track Progress</span>
          </div>
      ...
      <div className="student-dashboard-grid">
        {/* Left column: Calendar Widget + Milestones (condensed) */}
        ...
        <div className="student-dashboard-col">
          ...
          <Card>
            <CardHeader title="Milestones" icon={<HiOutlineAcademicCap />} iconBg="var(--status-purple-bg)" iconColor="var(--status-purple)"
              action={<button className="btn btn-primary btn-sm" onClick={() => setShowMilestoneModal(true)}><HiOutlinePlusCircle /> Add</button>} />
            <CardBody>
              {milestones.length === 0 ? (
                <EmptyState icon={<HiOutlineAcademicCap />} title="No milestones" description="Track progress by adding milestones" />
              ) : (
                milestones.slice(0, 5).map((ms) => (
                  <div key={ms.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border-light)' }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--status-purple)', flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ms.title}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{MILESTONE_TYPE_LABELS[ms.type]} · {formatDate(ms.date)}</div>
                    </div>
                    <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => openEditMilestone(ms)} title="Edit" style={{ padding: 4 }}><HiOutlinePencilSquare /></button>
                      <button className="btn btn-ghost btn-sm" onClick={() => handleDeleteMilestone(ms.id)} title="Delete" style={{ padding: 4, color: 'var(--status-danger)' }}><HiOutlineTrash /></button>
                    </div>
                  </div>
                ))
              )}
              {milestones.length > 5 && (
                <div style={{ textAlign: 'center', paddingTop: 8 }}>
                  <button className="btn btn-ghost btn-sm" onClick={() => navigate('/progress-tracker')}>View all {milestones.length} milestones <HiOutlineArrowRight /></button>
                </div>
              )}
            </CardBody>
          </Card>
